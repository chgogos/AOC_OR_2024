...
def solve(a_problem):
    ...
    logging.info(f"Total cost: {sum([data['weight'] for _, _, data in T.edges(data=True)])}")
    
    step = 0
    while True:
        step+=1
        # Remove leaf nodes from the minimum spanning tree
        leaves = [node for node in T.nodes() if T.degree(node) == 1 and node not in a_problem.mandatory_nodes]
        if len(leaves) == 0:
            break
        logging.info(f"Leaves that will be removed: {sorted(leaves)}")
        T.remove_nodes_from(leaves)
        logging.info(f"total cost at step {step}: {sum([data['weight'] for _, _, data in T.edges(data=True)])}")
        
    
    # Visualize the graph and the minimum spanning tree
    pos = nx.spring_layout(T)
    nx.draw_networkx_nodes(T, pos, nodelist=a_problem.mandatory_nodes, node_color="orange", node_size=700)
    nx.draw_networkx_nodes(T, pos, nodelist=[node for node in T.nodes if node not in a_problem.mandatory_nodes], node_color="lightblue", node_size=500)
